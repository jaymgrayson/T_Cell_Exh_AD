...
rm(list=ls())
library(tidyverse)
setwd("~/Desktop/Code copy/Figure 3/Panel_F")
load("~/Desktop/Code copy/Figure 3/Panel_F/CD8_PI_Stim.RDa")


g1<-ggplot(df, aes(x=df$Stimulation, y=df$`IL2 IFNg DP`)) +theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ ggtitle("IFNg IL-2 Producing CD8+ T Cells")+theme(plot.title = element_text(size=20)) + theme(plot.title = element_text(hjust = 0.5))+geom_boxplot()
