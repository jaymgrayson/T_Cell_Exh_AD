...
rm(list=ls())
library(tidyverse)
setwd("~/Desktop/Code copy/Figure 3/Panels_H_I")
load("~/Desktop/Code copy/Figure 3/Panels_H_I/CD8_Peptide_Man_ICS.RDa")

g1<-ggplot(df, aes(x=df$Stimulation, y=df$`IL2 neg IFNg SP`)) +theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ ggtitle("IFNg IL-2 neg Producing CD8+ T Cells")+theme(plot.title = element_text(size=20)) + theme(plot.title = element_text(hjust = 0.5))+geom_boxplot()
