...
load("~/Desktop/Code copy/Figure 2/Panel E/Box_Plot_Data_CD8.RDa")
#Plot cluster of interest
g1<-ggplot(df, aes(x=df$Disease_Classifier, y=df$`Cluster 7`)) +theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ ggtitle("Cluster 7")+theme(plot.title = element_text(size=20)) + theme(plot.title = element_text(hjust = 0.5))+geom_boxplot()
