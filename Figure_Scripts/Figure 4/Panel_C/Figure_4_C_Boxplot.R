...
rm(list=ls())
library(tidyverse)
setwd("~/Desktop/Code copy/Figure 4/Panel_C")
load("~/Desktop/Code copy/Figure 4/Panel_C/CD4_Perc_Phenogrpah_PI_Data.RDa")
g1<-ggplot(df, aes(x=df$Disease_Classifier, y=df$`Cluster 22`)) +theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ ggtitle("Cluster 22")+theme(plot.title = element_text(size=20)) + theme(plot.title = element_text(hjust = 0.5))+geom_boxplot()
